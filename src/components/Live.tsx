...
export default function Live() {
  ...
  const [chat, setChat] = useState([
    { id: 1, user: "SISTEMA", text: "Chat ao vivo! Envie sua dúvida." }
  ]);
  ...
  useEffect(() => {
    const users = ["MARIA", "ANA", "CARLA", "JULIANA", "ESTÉTICA PRO"];
    const texts = ["Amo esse brilho!", "Entrega no PR?", "O kit é top!", "Quero dois!", "Melhor produto!"];
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title: 'Beleza Link', url: window.location.href });
    } else {
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-24 font-sans">
      <nav className="fixed top-0 w-full z-[100] bg-black/95 border-b border-white/5 p-4 flex items-center justify-between px-6">
        <button onClick={() => window.history.back()} className="bg-[#F97316] text-black px-6 py-2 rounded-xl font-black text-[10px] uppercase flex items-center gap-2 shadow-lg">
          <ChevronLeft size={14} /> Voltar
        </button>
        <button onClick={handleShare} className="bg-white/5 text-white px-4 py-2 rounded-xl border border-white/10 text-[10px] font-bold uppercase flex items-center gap-2">
          <Share2 size={14} className="text-[#F97316]" /> Partilhar
        </button>
      </nav>
      ...
      <main className="px-4 md:px-12 max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 h-[500px] lg:h-[600px] bg-zinc-900/50 rounded-[40px] border border-white/10 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-white/5 bg-white/5 text-[10px] font-bold uppercase text-zinc-500 text-center">Chat em Tempo Real</div>
          ...
          <form onSubmit={handleSend} className="p-4 bg-black/40 border-t border-white/10 flex gap-2">
            <input value={msgInput} onChange={(e) => setMsgInput(e.target.value)} placeholder="Comentar..." className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#F97316]" />

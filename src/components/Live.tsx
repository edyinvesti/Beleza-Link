...
export default function Live() {
  ...
  // Simulação de pessoas mandando mensagem
  useEffect(() => {
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4 scrollbar-hide">
            {chat.map((c) => (
              <div key={c.id} className="flex flex-col">
                ...
                <span className="text-[13px] text-zinc-200 bg-white/5 p-3 rounded-2xl border border-white/5 inline-block">{c.text}</span>
              </div>
            ))}
            <div ref={chatEndRef} />
          </div>
          <form onSubmit={handleSend} className="p-4 bg-black/40 border-t border-white/10 flex gap-2">
            <input value={msgInput} onChange={(e) => setMsgInput(e.target.value)} placeholder="Escreva..." className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#F97316]" />

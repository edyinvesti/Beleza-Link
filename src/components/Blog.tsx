...
const allPosts = [
  { 
    ...
    category: "Masculino", 
    // Link corrigido com parâmetros de renderização direta
          ...
          {!selectedPost ? (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {allPosts.map((post) => (
                  <motion.article key={post.id} onClick={() => setSelectedPost(post)} className="cursor-pointer group relative bg-[#050505] rounded-[40px] border border-white/5 overflow-hidden transition-all">
                    <div className="h-[500px] bg-[#111]">
                      <img 
                        src={post.image} 
                        className="w-full h-full object-cover transition-transform duration-[2s] group-hover:scale-110" 
                        alt={post.title}
                        onError={(e) => { e.currentTarget.src = "https://via.placeholder.com/800x600/000000/F97316?text=BELEZA+LINK"; }}
                      />
                    </div>
            ...
            <motion.div key="reading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto">
               <button onClick={() => setSelectedPost(null)} className="mb-8 text-[#F97316] font-bold uppercase tracking-widest text-xs">← VOLTAR</button>
               <h2 className="text-5xl md:text-8xl font-black mb-16 leading-none tracking-tighter uppercase">{selectedPost.title}</h2>
               <img src={selectedPost.image} className="w-full rounded-[40px] mb-12 shadow-2xl" />
               <div className="text-white/80 text-xl md:text-2xl leading-relaxed space-y-12 text-justify pb-20">
                 {selectedPost.content}
                 <div className="pt-20">
                    <a href={selectedPost.affiliateLink} target="_blank" className="inline-block w-full bg-[#F97316] text-black text-center py-6 rounded-full font-black text-xl uppercase hover:bg-white transition-all">
